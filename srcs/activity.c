...
/*   Updated: 2023/10/06 19:28:24 by rr               ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int routine_eat(t_philo *philo)
{
	...
	if (display_philo_status(get_time(philo->data), philo->id, 'F', philo->data) == 0)
	...
	if (display_philo_status(get_time(philo->data), philo->id, 'E', philo->data) == 0)
		return (pthread_mutex_unlock(philo->left),
			pthread_mutex_unlock(philo->right), 0);
...
{
	if (display_philo_status(get_time(philo->data), philo->id, 'S', philo->data) == 0)
		return (0);
	...
	while (1)
	{
		...
		while (++i < philo->data->count_philo)
		{
				...
				|| (data->count_meal > 0 && data->finish == data->count_philo))
			{
				...
				if (philo->time > philo->data->time_dead)
					display_philo_status(philo->time, philo->id, 'D', philo->data);
...
void *routine(void *arg)
{
	...
	while (1)
	{
		...
		if (routine_sleep(data) == 0)
			break ;
		if (display_philo_status(get_time(data->data), data->id, 'T', data->data) == 0)
			break ;
	}
	return (NULL);
}
